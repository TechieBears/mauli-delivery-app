...
import { useVendorProfile, useKycStatus } from '../../hooks/useVendorQueries';
...
const VendorProfileDetailsScreen = ({ navigation }) => {
  const profile = useAppStore(state => state.profile);
  const user = useAppStore(state => state.user);
  const setKycStatus = useAppStore(state => state.setKycStatus);

  const { data: kycStatusRes, isLoading: loadingKyc, error: kycError } = useKycStatus();
  const kycStatus = kycStatusRes?.data;

  // The Vendor profile is needed both to display an approved vendor's details
  // and to derive the resume step for an incomplete (drafted/pending) one.
  const needsProfile = kycStatus === 'approved' || isOnboardingIncomplete(kycStatus);
  const { data: vendorProfileRes } = useVendorProfile(needsProfile);
  ...
  useEffect(() => {
    ...
    if (isOnboardingIncomplete(kycStatus)) {
      if (!vendor) return; // wait for the profile so we can derive the resume step
      navigation.replace('Onboarding', {
        role: 'vendor',
        phone: user?.phone ?? profile?.phone ?? '',
        resumeStep: getResumeStep(vendor),
      });
  ...
  }, [kycStatus, vendor, navigation, profile, user, setKycStatus]);
  ...
  // While loading, or while redirecting a non-approved vendor, show a spinner
  ...
  const email = vendor?.userId?.email || profile?.email || 'rajesh.farm@maulimart.com';
  const phone = vendor?.userId?.phone
    ? `${vendor.userId.countryCode || '+91'} ${vendor.userId.phone}`
    : profile?.phone || '+91 98765 43210';
  const address = vendor?.address?.line || profile?.address || 'Gat No. 45, Green Valley Farms, Nashik Highway, Pune, MH 411041';
  const bankName = vendor?.bankDetails?.bankName || profile?.bankName || 'State Bank of India';
  const accountNumber = vendor?.bankDetails?.accountNumber || profile?.accountNumber || '••••  ••••  8291';
  const ifscCode = vendor?.bankDetails?.ifscCode || profile?.ifscCode || 'SBIN0001234';
  const accountType = vendor?.bankDetails?.accountType
    ? vendor.bankDetails.accountType.charAt(0).toUpperCase() + vendor.bankDetails.accountType.slice(1)
    : profile?.accountType || 'Current';
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <SectionTitle title="PERSONAL & BUSINESS" />
        ...
        <View style={styles.card}>
          {vendor?.panCardNo || vendor?.panFile ? (
            <KycRow
              icon={<DocumentText1 size={17} color={colors.textSecondary} variant="Linear" />}
              label="PAN Card"
            />
          ) : null}
          {vendor?.gstNo || vendor?.gstFile ? (
            <KycRow
              icon={<DocumentText1 size={17} color={colors.textSecondary} variant="Linear" />}
              label="GST Certificate"
            />
          ) : null}
          {vendor?.fssaiNo || vendor?.fssaiFile ? (
            <KycRow
              icon={<DocumentText1 size={17} color={colors.textSecondary} variant="Linear" />}
